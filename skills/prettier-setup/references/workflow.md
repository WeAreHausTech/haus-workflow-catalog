...
3. Install both packages as devDependencies:
   - Yarn: `yarn add -D prettier @haus-tech/prettier-config`
   - pnpm: `pnpm add -D prettier @haus-tech/prettier-config`
   - npm: `npm i -D prettier @haus-tech/prettier-config`
4. Create `.prettierrc` containing exactly the single-line string `"@haus-tech/prettier-config"`.
...
cat package.json | jq '.devDependencies.prettier, .devDependencies["@haus-tech/prettier-config"]'
ls -la .prettierrc* prettier.config.* 2>/dev/null

# Install
yarn add -D prettier @haus-tech/prettier-config
...
- [ ] `prettier` and `@haus-tech/prettier-config` in `devDependencies`
- [ ] Exactly one Prettier config file at repo root (`.prettierrc`)
- [ ] `.prettierrc` content is the single-string `"@haus-tech/prettier-config"` reference
